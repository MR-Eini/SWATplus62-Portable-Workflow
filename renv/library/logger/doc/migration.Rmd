...
Both [`loggit`](https://cran.r-project.org/package=loggit) and [`loggr`](https://github.com/smbache/loggr) provides an easy way of capturing `message`, `warning` and `stop` function messages. Similar functionality has been added to `logger` in the form of `log_message`, `log_warning` and `log_error` functions.
